...
public class An11 {
    ...
    void testDoiXung() {
        ...
        boolean result = x.kiemTraDoiXung(112);
        check("number=112", "kiemTraDoiXung(112)", String.valueOf(result), "false");
        assertFalse(result);
    }

    @Test
    void testDoiXungTrue() {
        Advance5 x = new Advance5();
        boolean result = x.kiemTraDoiXung(12121);
        check("number=12121", "kiemTraDoiXung(12121)", String.valueOf(result), "true");
        assertTrue(result);
    }
    ...
    void testZero() {
        Advance5 x = new Advance5();
        boolean result = x.kiemTraDoiXung(0);
        check("number=0", "kiemTraDoiXung(0)", String.valueOf(result), "true");
        assertTrue(result);
    }

    @Test
    void testNegativeFalse() {
        Advance5 x = new Advance5();
        boolean result = x.kiemTraDoiXung(-102);
        check("number=-102", "kiemTraDoiXung(-102)", String.valueOf(result), "false");
        assertFalse(result);
    }

    @Test
    void testNegativeTrue() {
        Advance5 x = new Advance5();
        boolean result = x.kiemTraDoiXung(-101);
        check("number=-101", "kiemTraDoiXung(-101)", String.valueOf(result), "true");
        assertTrue(result);
    }
    ...
    static void exportReport() throws IOException {
        ...
        html.append("<!DOCTYPE html><html><head><meta charset='UTF-8'><style>body{font-family:Arial;padding:30px;}h2{color:#333;}table{border-collapse:collapse;width:100%;}th{background:#4CAF50;color:white;padding:10px;border:1px solid #ccc;}td{border:1px solid #ccc;padding:8px;text-align:center;}tr:nth-child(even){background:#f9f9f9;}.pass{color:green;font-weight:bold;}.fail{color:red;font-weight:bold;}</style></head><body>")
            .append("<h2>Test Report - Question 11: Advance5 (kiemTraDoiXung)</h2>")
        ...
        try (FileWriter fw = new FileWriter("test-report-q11.html")) { fw.write(html.toString()); }
        System.out.println(">>> Report saved: test-report-q11.html");
    }
}